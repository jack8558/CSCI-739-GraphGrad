...
{
public:
    BinaryOp(std::shared_ptr<Tensor> arg1, std::shared_ptr<Tensor> arg2, BinaryOpType op_type) : Tensor(get_dims(arg1, arg2, op_type)), leftChild(arg1), rightChild(arg2), op_type(op_type) {}
...
protected:
    ...
    std::vector<size_t> get_dims(std::shared_ptr<Tensor> left, std::shared_ptr<Tensor> right, BinaryOpType op_type){
        switch (op_type)
        {
        case BinaryOpType::MATMUL:
