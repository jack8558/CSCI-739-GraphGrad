...
def torch_tensor2(gg_tensor2):
    tensor = torch.tensor(gg_tensor2.to_list(), dtype=torch.float64)
...
class TestBinaryOP:
    ...
    def test_mult(self, gg_tensor, torch_tensor):
        assert np.isclose(gg.mult(gg_tensor, 7).to_list(), 
                          torch.mult(torch_tensor, 7).tolist(), rtol=1e-4).all()
    ...
    def test_elementwise_mult(self, gg_tensor, torch_tensor, gg_tensor2, torch_tensor2):
        assert np.isclose(gg.elementwise_mult(gg_tensor, gg_tensor2).to_list(), 
                          torch.mult(torch_tensor, torch_tensor2).tolist(), rtol=1e-4).all()
    ...
    def test_matmul(self, gg_tensor, torch_tensor, gg_tensor2, torch_tensor2):
        assert np.isclose(gg.matmul(gg_tensor, gg_tensor2).to_list(), 
                          torch.matmul(torch_tensor, torch_tensor2).tolist(), rtol=1e-4).all()
